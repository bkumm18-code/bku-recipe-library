...
- Add the supplied blue BKU logo to `H6`, at `81 × 41 px`, on every invoice tab.
- Add the supplied red BKU seal to `K6`, at `100 × 99 px`, on every invoice tab.
- Insert the logos as images only. Do not change any cell value, formula, formatting, merge, row height, column width, or print setting when adding or repositioning them.
...
- Leave the template's transaction-date cells (`B21:B35`, below `取引日`) blank. Use the source end date only inside the Period field; do not render it as a detail-row transaction date.
...
- Later lines: eligible trainee names from the matching `mNNN` worksheet, separated with real Excel line feeds; enable WrapText. Prefix those names sequentially as `01 `, `02 `, `03 `, and so on; do not prefix the Japanese company-name first line.
- Populate one complete detail row for every eligible source record, including records that fall on later template rows. A populated detail row must have its company-and-trainee summary in column C and its round, entry date, start/end dates, quantity, headcount, unit price, and amount in columns G:K; the transaction-date cell in column B remains blank.
...
- When the user explicitly supplies a corrected trainee name for a specific invoice row, use that spelling in the output even if it differs from the trainee workbook. Change only that name line; preserve the company line, sequential number, and every other existing detail value.
...
- Japanese-only company summary, sequentially numbered eligible trainee names, integer round, entry/start/end dates, final-pax headcount, and line breaks.
- Blank detail transaction-date cells (`B21:B35`).
- A complete C and G:K detail record for every eligible source row; do not omit the final rows for a cooperative.
- Any user-specified trainee-name correction appears on the requested sheet and row without changes to unrelated detail fields.
